...
import matplotlib.pyplot as plt  # 确保已导入
...
def main():
    ...
    for i in range(500):
        ...
        if i % 5 == 0:
            test_losses.append(test_loss.item())  # 记录 test_loss
            epochs.append(i)  # 记录当前 epoch
            print('epoch[{}]  train_loss = {:0.3f} train_loss_2 = {:0.3f}  test_loss = {:0.3f}  '.format(
                i, train_loss, train_loss_2, test_loss))

    # 保存结果
    util.write_data('4_spe_train', spe_train.data.cpu().numpy())
    util.write_data('4_spe_test', spe_test.data.cpu().numpy())
    util.write_data('4_T2_train', T2_train.data.cpu().numpy())
    util.write_data('4_T2_test', T2_test.data.cpu().numpy())
