...
class MLP(torch.nn.Module):
    ...
    def predict(self, x):
        ...
        with torch.no_grad():
            out = self.a(self.h(x))
            pred_x = self.o(self.a(self.h(x)))
            loss = torch.mean(torch.pow(pred_x - x, 2))
            loss_single = torch.mean(torch.pow(pred_x - x, 2), dim=1)
...
def main():
    train_dataset = torch.from_numpy(get_train_data())
    test_dataset = torch.from_numpy(get_test_data())
    ...
    #optimizer = torch.optim.SGD(model.parameters(), lr=0.5, momentum=0.9, weight_decay=0.005)
